  ...
  <div class="user" >
    ...
    <div class="scroll" >
      <cube-scroll
        ...
        <!-- 用户关于 -->
        <ul class="user-about" >
          <li v-for="(about, index) in aboutList" :key="index" @click="aboutClick(about.name, index)"
              class="btn-about" >
        ...
        <ul class="user-about user-private" >
          <li v-for="(about, index) in privateList" :key="index" @click="aboutClick(about.name, index)"
              class="btn-about" >
            <div class="icon" :class="about.pic" ></div >
            <div class="text" >{{about.name}}</div >
          </li >
    ...
    <!-- 滚动结束 -->
    <div
      style="display: block;position: fixed;top: 0;z-index: 99999;width: 100%;max-width: 750px;max-height: 50vh;background-color: rgba(0,0,0,.5);color: #fff" >
